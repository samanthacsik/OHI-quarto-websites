...
5. **Note:** I've moved my `title` beneath `navbar` (by default, it's placed beneath `website` at the same level as `navbar`) -- I think this makes a bit more intuitive sense, since your website `title` is positioned within your `nabar` (or `sidebar`). This new positioning is also important if we decide to add both a website title *and* logo (we'll explore this in a later section).
