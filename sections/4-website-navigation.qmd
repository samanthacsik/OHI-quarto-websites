...
format:
  html:
    ...
    toc: true
```
1. Set your navbar pages on the `left` or `right`
...
A sidebar is a great option if you have *lots* of documents which require a deep hierarchy of pages.
...
format:
  html:
    ...
    toc: true
```
1. Choose from `"docked"` or `"floating"`
2. Remove search bar by setting `style` to `false`
3. All top level pages or sections fall under this first `contents` section
4. Create a sub menu by adding `contents` beneath a `section` header
...
::: {.center-text .gray-text}
*Newly added sidebar pages with the "Models" section expand to reveal sub menu pages.*
:::

Let's say we want to recreate [this sub menu](https://oceanhealthindex.org/images/htmls/Supplement.html#53_Likely_future_status_dimensions) as it appears in the [Ocean Health Index 2022: Methods](https://oceanhealthindex.org/images/htmls/Supplement.html) document. I'll first add three additional pages, `trend.qmd`, `pressure.qmd`, `resilience.qmd`, which we'll include as sub menu items beneath the section header, **Likely future status dimensions**:
...
format:
  html:
    ...
    toc: true
```
1. Here, we converted the old `future-dimensions.qmd` into a section header with the same title, **Likely future status dimensions**. Expanding it reveals three new pages, **Trend**, **Pressure**, and **Resilience**.
...
We can add additional sub menus following the pattern above.
